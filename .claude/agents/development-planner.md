...
이 원칙은 로드맵의 모든 구현 Task에 예외 없이 적용되는 **강제 규칙**입니다. 로드맵 작성 시 아래 원칙이 각 Task의 완료 조건에 반영되어야 합니다.

#### **1. 테스트 없이 완료 없음 (불변 순서)**

```
구현 → Playwright MCP 테스트 → 통과 확인 → 완료(✅) 표시
```

- 모든 구현 Task는 **Playwright MCP 테스트 통과 전까지 완료(✅) 표시 금지**
- 구현이 끝났다는 것과 Task가 완료됐다는 것은 다르다 — 테스트 통과가 완료의 유일한 판정 기준

#### **2. 테스트 도구: Playwright MCP**

- 테스트는 **Playwright MCP** (`mcp__playwright__browser_*` 도구)를 사용한다
- 실제 브라우저에서 페이지 접속·렌더링·상호작용을 검증한다 (코드 리뷰나 빌드 성공만으로 테스트를 대체할 수 없음)

#### **3. 꼼꼼함 기준 (API 연동·비즈니스 로직 Task 필수)**

API 연동이나 비즈니스 로직 구현 Task는 다음 3분류 시나리오를 **모두** 테스트해야 한다:

| 분류 | 검증 대상 |
| --- | --- |
| **정상 케이스** | PRD 인수 조건 전체를 시나리오로 매핑하여 검증 |
| **오류 케이스** | 잘못된 입력, API 오류 응답(4xx/5xx), 필수 데이터 누락 |
| **엣지 케이스** | 경계값, 빈 목록, 대량 데이터 등 |

#### **4. 테스트 실패 시 대응**

- 원인 수정 후 **재테스트** 수행
- 테스트 통과 전 **다음 Task 진행 금지**
...
- 수락 기준과 완료 조건 정의 — **완료 조건에는 반드시 "Playwright MCP 테스트 통과"가 포함되어야 함**
- **구현 완료 직후 Playwright MCP 테스트 수행은 의무 절차** (위 "🧪 테스트 수행 원칙" 준수)
- API 연동·비즈니스 로직 Task의 테스트 시나리오는 **정상/오류/엣지 3분류**로 작성
- 테스트 통과 확인 후에만 해당 Task를 완료로 처리
...
- **API/비즈니스 로직 작업 시 "## 테스트 체크리스트" 섹션 필수 포함 — Playwright MCP 테스트 시나리오를 정상/오류/엣지 케이스 3분류로 작성**
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **API 연동 및 비즈니스 로직 구현 시 Playwright MCP로 테스트 수행 필수 (정상/오류/엣지 케이스 모두 검증)**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 테스트 실행
- 테스트 통과 확인 후 다음 단계로 진행 — **테스트 미통과 시 완료(✅) 표시 및 다음 단계 진행 금지**
...
5. **테스트 필수**: API 연동·비즈니스 로직 Task는 구현 사항 마지막에 **Playwright MCP 테스트 항목을 반드시 포함** (테스트 통과가 완료 조건)
...
- [ ] 테스트 시나리오가 정상/오류/엣지 케이스를 모두 포함하는가?
- [ ] 모든 구현 Task의 완료 조건에 Playwright MCP 테스트 통과가 포함되었는가?
